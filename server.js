...
const ORDER_NOW_TEXT = `Dạ em nhận đơn cho anh/chị ngay ạ.
Anh/chị gửi giúp em:
- loại gạo
- số kg
- tên người nhận
...
function getOrderFieldPrompt(field) {
  ...
  if (field === "quantityKg") {
    return "Dạ anh/chị cần bao nhiêu kg ạ?";
  }
...
function buildRestaurantRiceReply() {
  return {
    text: `Dạ nếu anh/chị mua cho quán ăn, em sẽ tư vấn theo tiêu chí ngon ổn định và giá hợp lý ạ.
...
async function buildBotResponse(session, messageText, menuPayload) {
  ...
  if (isDirectCallRequest(messageText)) {
    resetOrder(session);
    return {
      text: "Dạ anh/chị có thể liên hệ hotline/Zalo: 0762234135 ạ. Nếu muốn, anh/chị để lại số điện thoại, bên em gọi lại cho anh/chị ngay nhé.",
      includeMenu: true,
    };
  }
